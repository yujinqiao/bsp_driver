...
void  TEMP_ADC_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(TEMP_RCC|RCC_APB2Periph_ADC2, ENABLE );	  //使能ADC2通道时钟
	RCC_ADCCLKConfig(RCC_PCLK2_Div6);   //设置ADC分频因子6 72M/6=12,ADC最大时间不能超过14M
   
	//PA0 作为模拟通道输入引脚                         
    //    RCC_APB2PeriphClockCmd(MQ2_RCC,ENABLE);//使能PORTF时钟		
	GPIO_InitStructure.GPIO_Pin = TEMP_PIN;//PA0 anolog输入
...
void  HUMI_ADC_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(HUMI_RCC|RCC_APB2Periph_ADC1	, ENABLE );	  //使能ADC1通道时钟
    ...
    //    RCC_APB2PeriphClockCmd(MQ2_RCC,ENABLE);//使能PORTF时钟		
	GPIO_InitStructure.GPIO_Pin = HUMI_PIN;//PA0 anolog输入
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;		//模拟输入引脚
